...
<<<<<<< HEAD
    ...
    _license_hash: str = ""
=======
    # 2. 尝试 Cython (.so/.pyd) — 中等安全
    try:
        from app.core.feature_gate import check_feature, set_license, get_tier, reset as _reset
        # Cython Pro modules
        from app.core.memory_decay import calculate_decay, should_prune, reinforce, decay_memory, decay_batch, get_decay_stats
        from app.core.conflict_resolver import detect_conflict, resolve_conflict, scan_for_conflicts, get_conflict_summary
        from app.core.token_router import estimate_complexity, route_model, get_routing_stats
        _CORE_ENGINE = "cython"
        USING_CYTHON = True
        logger.info("Core engine: Cython — moderate security")
    except ImportError:
        USING_CYTHON = False

        # 3. 纯 Python 兜底 — 最低安全 (仅开发环境)
...
=======
        def _verify_integrity() -> bool:
            ...
            if not _license_hash:
                return _tier == "oss"
            raw = f"{_tier}|{','.join(sorted(_features))}"
            expected = hashlib.sha256(raw.encode()).hexdigest()[:16]
            return _license_hash == expected
        ...
        _CORE_ENGINE = "python"
        logger.warning("Core engine: Pure Python — LOW security, install clawmemory-core for protection")
>>>>>>> fb055c7 (feat: v3.0 - Wiki知识库 + 科技感UI + i18n + Rust PyO3核心 + Pro功能)
...
<<<<<<< HEAD
                    json={"license_key": license_key, "fingerprint": self._get_fingerprint(), "version": "1.2.0"},
=======
...
>>>>>>> fb055c7 (feat: v3.0 - Wiki知识库 + 科技感UI + i18n + Rust PyO3核心 + Pro功能)
        ...
        license_type = data.get("license_type", "pro_annual")
    ...
    def load_cached_license(self):
        ...
        if lic:
            ...
            tier = "pro" if lic.tier in ("pro_annual", "pro_lifetime") else lic.tier
            set_license(tier, features)
