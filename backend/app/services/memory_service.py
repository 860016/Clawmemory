...
class MemoryService:
    ...
    def list_memories(self, layer: str | None = None, page: int = 1, size: int = 20) -> tuple[list[Memory], int]:
    ...
    def search_keywords(self, query: str, limit: int = 20) -> list[dict]:
        ...
        results = self.db.execute(
            "SELECT m.id, m.key, m.value, m.layer, m.source, f.rank as score "
            "FROM memories_fts f JOIN memories m ON m.id = f.rowid "
            "WHERE f.memories_fts MATCH :query AND m.user_id = 1 "
