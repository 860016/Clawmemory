...
func RegisterRoutes(r *gin.Engine, db *gorm.DB) {
	...
	public := r.Group("/api/v1")
	{
		...
		public.POST("/license/activate", middleware.LoginRateLimit(), handleLicenseActivate(licenseManager, proProxy))
	}
	...
	authorized.Use(middleware.Auth(cfg, db), middleware.JWTRateLimit())
	{
		...
		authorized.POST("/license/deactivate", handleLicenseDeactivate(licenseManager, proProxy))
		...
		authorized.GET("/openclaw/agents-md", handleGetAgentsMD(db))
