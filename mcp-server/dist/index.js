...
server.tool("memory_search", "Search ClawMemory for relevant memories. Use this when you need to recall past information, user preferences, or context from previous conversations.", {
    ...
    try {
        ...
        const lines = items.map((m) => `- [${m.source || "?"}] ${m.key}: ${m.value}`);
        return {
...
server.tool("memory_context", "Get contextual memories with a pre-formatted system prompt addition. Use this to inject relevant memories into your context before responding.", {
...
server.tool("memory_reason", "Perform dialectic reasoning about the user using their configured AI model. This analyzes conversation patterns and derives insights about user preferences, goals, and working style. Unlike Honcho's paid Dialectic Reasoning, this uses YOUR own AI model at no extra cost.", {
    ...
    try {
        ...
        return {
            content: [{ type: "text", text: result.reasoning || "No reasoning result" }],
        };
    }
    catch (err) {
        const msg = err instanceof Error ? err.message : String(err);
...
server.tool("memory_push_conversation", "Push a conversation turn to ClawMemory for persistent storage. Use this to save important conversation exchanges.", {
